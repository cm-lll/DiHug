...
LOG_DIR="logs/logs_context_decoder_train300"
mkdir -p "$LOG_DIR"
...
COMMON=(
  ...
  model.partition_context_cross_edge_ratio=0.1
  model.partition_context_edge_budget=0
  ...
  model.sampling_partition_ensemble_diag_max_candidates=200000
...
launch() {
  ...
  local seed="$4"
  local tag="$5"
  local name="pubmed_ef010_ctxdecode_${tag}_seed${seed}_ep300"

  CUDA_VISIBLE_DEVICES="$gpu" CONDA_ENV=sparse_block \
    nohup bash scripts/run_pubmed_baseline_single.sh \
      "${COMMON[@]}" \
      "train.seed=$seed" \
      "model.train_partition_ensemble_metis_weight=$weight" \
