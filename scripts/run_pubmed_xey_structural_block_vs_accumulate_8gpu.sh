...
launch "${GPUS[0]}" xey_struct_block_bce \
  model.train_all_blocks_step_mode=sequential

launch "${GPUS[1]}" xey_struct_block_dpjs \
  model.train_all_blocks_step_mode=sequential \
  model.degree_pair_dist_loss_weight=0.05 \
  model.degree_pair_dist_loss_t_power=1.0 \
  model.degree_pair_dist_loss_warmup_epochs=5

launch "${GPUS[2]}" xey_struct_block_closure \
  model.train_all_blocks_step_mode=sequential \
  model.closure_rank_loss_weight=0.05 \
  model.closure_rank_loss_t_power=1.0 \
  model.closure_rank_loss_warmup_epochs=5 \
...
launch "${GPUS[3]}" xey_struct_block_combo_mild \
  "${BLOCK_MILD[@]}"

launch "${GPUS[4]}" xey_struct_global_bce \
  model.train_all_blocks_step_mode=accumulate_streaming_global

launch "${GPUS[5]}" xey_struct_global_dpjs \
  model.train_all_blocks_step_mode=accumulate_streaming_global \
  model.degree_pair_dist_loss_weight=0.05 \
  model.degree_pair_dist_loss_t_power=1.0 \
  model.degree_pair_dist_loss_warmup_epochs=5

launch "${GPUS[6]}" xey_struct_global_closure \
  model.train_all_blocks_step_mode=accumulate_streaming_global \
  model.closure_rank_loss_weight=0.05 \
  model.closure_rank_loss_t_power=1.0 \
  model.closure_rank_loss_warmup_epochs=5 \
...
launch "${GPUS[7]}" xey_struct_global_combo_mild \
  "${GLOBAL_MILD[@]}"
