...
LOG_DIR="${LOG_DIR:-logs/logs_hetero_xey_block_modes_6gpu}"
EPOCHS="${EPOCHS:-100}"
...
launch() {
  ...
  local mode="$2"
  local seed="$3"
  local name="hetero_xey_${mode}_seed${seed}_ep${EPOCHS}_skip10"
  ...
  echo "[launch] gpu=${gpu} mode=${mode} seed=${seed} name=${name}"
  (
...
launch 0 accumulate 0
launch 1 accumulate 1
launch 2 sequential 0
launch 3 sequential 1
launch 4 single 0
launch 5 single 1

echo "[launch] submitted 6 jobs. Logs: $ROOT/$LOG_DIR"
wait
echo "[done] all hetero X/E/y block-mode jobs finished. Logs: $ROOT/$LOG_DIR"
