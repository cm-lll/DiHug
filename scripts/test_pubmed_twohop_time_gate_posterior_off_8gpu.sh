...
LOG_DIR="${LOG_DIR:-logs/logs_twohop_time_gate_posterior_off}"
mkdir -p "$LOG_DIR"
...
launch() {
  ...
  EXPERIMENT=pubmed_query_gate_twohop_pw20 \
    bash scripts/run_pubmed_baseline_single.sh \
      ...
      "general.name=test_${run_name}_posterior_off" \
      ...
      >"$LOG_DIR/${run_name}_posterior_off.out" 2>&1 &
}
...
wait
echo "[done] posterior-off tests finished; logs: $LOG_DIR"
