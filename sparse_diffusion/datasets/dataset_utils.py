...
def resolve_data_path(datadir: str, original_cwd: str | None = None) -> str:
    """Resolve dataset paths for both fresh clones and older external-data layouts.

    Relative paths such as ``data/ACM_subgraphs`` should resolve inside the
    repository root for a new user.  For compatibility with older runs, we also
    accept the project parent directory if the data already exists there.
    """
    if osp.isabs(datadir):
        return datadir

    repo_root = pathlib.Path(__file__).resolve().parents[2]
    cwd = pathlib.Path(original_cwd).resolve() if original_cwd else repo_root
    candidates = [
        cwd / datadir,
        repo_root / datadir,
        cwd.parent / datadir,
        repo_root.parent / datadir,
    ]
    for candidate in candidates:
        if candidate.exists():
            return str(candidate)
    return str(repo_root / datadir)
