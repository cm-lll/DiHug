...
class ACMSubgraphsDataset(InMemoryDataset):
    ...
    def __init__(self, split: str, root: str, transform=None, pre_transform=None, heterogeneous: bool = True):
        ...
        self.data, self.slices = load_pt(self.processed_paths[0])  # 用 processed_paths[0]（即当前 split 的 .pt）加载整份图到 self.data 和 self.slices，这样 len(self) 和 self[i] 就都由基类按切片正确工作。
    ...
    def process(self):
        ...
        nodes0 = load_pt(osp.join(chosen_dirs[0], "nodes.pt"), map_location="cpu")
        ...
        for sd in chosen_dirs:
            meta = _load_meta(sd)
            nodes = load_pt(osp.join(sd, "nodes.pt"), map_location="cpu")
            edges = load_pt(osp.join(sd, "edges.pt"), map_location="cpu")
...
class SingleGraphWrapper:
    """单图模式：从 ACM 某 split 取第 index 张图，train/val/test 都只返回这一张（和 DBLP 单图用法一致）。
    需要 root/processed_dir/statistics 是因为 ACMSubgraphsInfos 会从 datamodule.inner 读这些；
    __add__ 用 ConcatDataset 是因为 AbstractDataModule.dataset_stat() 会做 train_dataset + val_dataset + test_dataset 再迭代。
    """

    def __init__(self, source_dataset, index: int):
        self._source_dataset = source_dataset
        self._graph = source_dataset[index]
        self._statistics = source_dataset.statistics
    ...
    @property
    def root(self):
        return self._source_dataset.root

    @property
    def processed_dir(self):
        return self._source_dataset.processed_dir
...
class ACMSubgraphsDataModule(AbstractDataModule):
    def __init__(self, cfg):
        ...
        single_graph = getattr(cfg.dataset, "single_graph", False)
        single_graph_split = getattr(cfg.dataset, "single_graph_split", "train")
        single_graph_index = getattr(cfg.dataset, "single_graph_index", 0)

        if single_graph:
            # 只取一张图：从指定 split 取第 index 张，train/val/test 都用它
            source = ACMSubgraphsDataset(split=single_graph_split, root=root_path, pre_transform=pre_transform, heterogeneous=heterogeneous)
            n = len(source)
            if single_graph_index >= n:
                raise ValueError(
                    f"single_graph_index={single_graph_index} but {single_graph_split} has only {n} graphs. "
                    f"Use 0 <= index < {n}."
                )
            one = SingleGraphWrapper(source, single_graph_index)
            datasets = {"train": one, "val": one, "test": one}
            self.statistics = {"train": one.statistics, "val": one.statistics, "test": one.statistics}
            print(f"[ACM] single_graph=True: using {single_graph_split}[{single_graph_index}] for train/val/test (1 graph, {one._graph.x.shape[0]} nodes, {one._graph.edge_index.shape[1]} edges)")
        else:
            datasets = {
