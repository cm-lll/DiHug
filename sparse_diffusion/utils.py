...
def load_pt(path, map_location=None, **kwargs):
    """Load trusted local .pt files (PyG Data, splits, checkpoints with full state)."""
...
def get_model_output_root():
    """
    Base directory for all SparseDiff model artifacts (checkpoints, logs, graphs, chains, etc.).

    By default this is 'output/sparse_diffusion' under the project root, but you can override it
    via the SPARSEDIFF_MODEL_DIR environment variable if needed.
    """
    return os.getenv("SPARSEDIFF_MODEL_DIR", os.path.join("output", "sparse_diffusion"))


def create_folders(args):
    """
    Create per-experiment folders for figures/chains under the unified model output root.
    """
    base_dir = get_model_output_root()
    graphs_root = os.path.join(base_dir, "graphs")
    chains_root = os.path.join(base_dir, "chains")
    ...
    try:
        os.makedirs(os.path.join(graphs_root, args.general.name), exist_ok=True)
        os.makedirs(os.path.join(chains_root, args.general.name), exist_ok=True)
    except OSError:
        pass
