    ...
    def message(
        ...
        if int(os.environ.get("SPARSEDIFF_MEM_DEBUG", "0")):
            try:
                ...
                m, h, c = Y.shape
                bytes_per = Y.element_size()
                size_y = m * h * c * bytes_per / (1024 ** 2)
                size_mul = edge_attr_mul.numel() * edge_attr_mul.element_size() / (1024 ** 2)
                size_add = edge_attr_add.numel() * edge_attr_add.element_size() / (1024 ** 2)
                print(
                    f"[MEM][before_message] alloc={alloc:.2f}GB reserved={reserved:.2f}GB | "
                    f"Y={size_y:.1f}MB edge_mul={size_mul:.1f}MB edge_add={size_add:.1f}MB",
                    flush=True,
                )
            except Exception:
                pass
