...
class SamplingMolecularMetrics(nn.Module):
    ...
    def forward(self, generated_graphs: list, current_epoch, local_rank):
        ...
        # Save in any case in the graphs folder (under unified model output root)
        graphs_root = os.path.join(utils.get_model_output_root(), "graphs")
        os.makedirs(graphs_root, exist_ok=True)
        textfile = open(
            os.path.join(
                graphs_root,
                f"valid_unique_molecules_e{current_epoch}_GR{local_rank}.txt",
            ),
            "w",
        )
        ...
        csv_dir = os.path.join(utils.get_model_output_root(), "smiles")
        os.makedirs(csv_dir, exist_ok=True)
        csv_filename_pandas = os.path.join(csv_dir, f"smiles_data_{time_str}.csv")
        # Save the DataFrame to a CSV file (under output/sparse_diffusion/smiles)
