...
class SamplingMetrics(nn.Module):
    ...
    def reset(self):
        # 仅保留 EdgeTypesTV + 结构对比指标（graph_comparison_metrics 直接按 batch 计算，不需要 metric 累加器）
        self.edge_types_tv.reset()
    ...
    def compute_all_metrics(self, generated_graphs: list, current_epoch, local_rank, key_suffix="", chart_title_suffix="", true_conditioned_pred_edge_counts=None):
        """Sampling/eval metrics (trimmed):
        - EdgeTypesTV
        - Graph structural comparison metrics:
          Clustering_real/gen, Triangles_real/gen, LCCSize_real/gen, EdgeOverlapRate,
          PowerLawAlpha_real/gen, DegreeAssortativity_real/gen, DegreeMMD
        """
        stat = (
        ...
        _bond_types = stat.bond_types
        if not isinstance(_bond_types, torch.Tensor):
            _bond_types = torch.from_numpy(np.asarray(_bond_types)).float()

        # Edge types TV (no charts)
        ...
        if local_rank == 0:
            ...
            keep = {
                f"{key}{key_suffix}/Graph/Clustering_real",
                f"{key}{key_suffix}/Graph/Clustering_gen",
                f"{key}{key_suffix}/Graph/Triangles_real",
                f"{key}{key_suffix}/Graph/Triangles_gen",
                f"{key}{key_suffix}/Graph/LCCSize_real",
                f"{key}{key_suffix}/Graph/LCCSize_gen",
                f"{key}{key_suffix}/Graph/Edges_real",
                f"{key}{key_suffix}/Graph/Edges_gen",
                f"{key}{key_suffix}/Graph/AvgDegree_real",
                f"{key}{key_suffix}/Graph/AvgDegree_gen",
                f"{key}{key_suffix}/Graph/EdgeOverlapRate",
                f"{key}{key_suffix}/Graph/PowerLawAlpha_real",
                f"{key}{key_suffix}/Graph/PowerLawAlpha_gen",
                f"{key}{key_suffix}/Graph/DegreeAssortativity_real",
                f"{key}{key_suffix}/Graph/DegreeAssortativity_gen",
                f"{key}{key_suffix}/Graph/DegreeMMD",
            }
            for k, v in cmp_metrics.items():
                if k in keep:
                    to_log[k] = v
...
def node_types_distance(generated_graphs, target, save_histogram=True, dataset_infos=None, chart_title_suffix="", current_epoch=None):
    ...
    # 应与 dataset 的 data.x（全局子类别 id 0..num_classes-1）及 atom_type_counts 的索引一致
    ...
    # 调试：若需排查节点分布差异，可设置 SPARSEDIFF_DEBUG_NODE_DIST=1 打印 target vs generated（归一化）
    if os.environ.get("SPARSEDIFF_DEBUG_NODE_DIST"):
        import sys
        g_sum = generated_distribution.sum().item()
        t_sum = target.sum().item()
        gen_n = (generated_distribution.float() / g_sum).tolist() if g_sum > 0 else [0.0] * n_classes
        tgt_n = (target.float() / t_sum).tolist() if t_sum > 0 else [0.0] * n_classes
        names = getattr(dataset_infos, "node_subtype_names", None) if dataset_infos else None
        print("[NodeTypesTV debug] index | target(norm) | generated(norm) | name", file=sys.stderr)
        for i in range(n_classes):
            name = (names[i] if names and i < len(names) else str(i))
            print(f"  {i} | {tgt_n[i]:.5f} | {gen_n[i]:.5f} | {name}", file=sys.stderr)
...
def _set_structural_stats(graphs):
    ...
    for g in graphs:
        if g.number_of_nodes() == 0:
            continue
        edge_counts.append(float(g.number_of_edges()))
        avg_degrees.append(float(2.0 * g.number_of_edges() / max(g.number_of_nodes(), 1)))
    ...
    return {
        ...
        "edge_count": _safe_mean(edge_counts),
        "avg_degree": _safe_mean(avg_degrees),
...
def format_structure_metrics_table(metrics_dict, key_prefix="test"):
    """Format graph structure comparison metrics as a 真实/生成 table string.
    metrics_dict: dict with keys like {key_prefix}/Graph/Clustering_real, _gen, _abs_gap, etc.
    """
    base = f"{key_prefix}/Graph"
    pairs = [
        ("Clustering coefficient (聚类系数)", f"{base}/Clustering_real", f"{base}/Clustering_gen", f"{base}/Clustering_abs_gap"),
        ("Triangle count (三角形数)", f"{base}/Triangles_real", f"{base}/Triangles_gen", f"{base}/Triangles_abs_gap"),
        ("LCC size (最大连通分量)", f"{base}/LCCSize_real", f"{base}/LCCSize_gen", f"{base}/LCCSize_abs_gap"),
        ("Edge count (边数)", f"{base}/Edges_real", f"{base}/Edges_gen", f"{base}/Edges_abs_gap"),
        ("Average degree (平均度)", f"{base}/AvgDegree_real", f"{base}/AvgDegree_gen", f"{base}/AvgDegree_abs_gap"),
        ("Edge Overlap Rate (边重叠率)", None, f"{base}/EdgeOverlapRate", None),
        ("Powerlaw alpha (幂律系数)", f"{base}/PowerLawAlpha_real", f"{base}/PowerLawAlpha_gen", f"{base}/PowerLawAlpha_abs_gap"),
        ("Degree Assortativity (度同配)", f"{base}/DegreeAssortativity_real", f"{base}/DegreeAssortativity_gen", f"{base}/DegreeAssortativity_abs_gap"),
        ("Degree MMD (度分布MMD)", None, f"{base}/DegreeMMD", None),
        ("Connectivity Agreement (连通性一致%)", None, f"{base}/ConnectivityAgreement", None),
    ]
    lines = [
        "",
        "========== 结构指标对比 (真实 vs 生成) ==========",
        f"{'指标':<36} {'真实':>12} {'生成':>12} {'差距':>12}",
        "-" * 76,
    ]
    for name, k_real, k_gen, k_gap in pairs:
        real_v = metrics_dict.get(k_real, None)
        gen_v = metrics_dict.get(k_gen, None)
        gap_v = metrics_dict.get(k_gap, None) if k_gap else None
        if real_v is None and gen_v is None:
            continue
        def fmt(v):
            if v is None or (isinstance(v, float) and (np.isnan(v) or v == -1.0)):
                return "-"
        ...
        real_s = fmt(real_v)
        gen_s = fmt(gen_v)
        gap_s = fmt(gap_v) if gap_v is not None else "-"
        lines.append(f"{name:<36} {real_s:>12} {gen_s:>12} {gap_s:>12}")
    lines.append("=" * 76)
    return "\n".join(lines)
...
def graph_comparison_metrics(generated_graphs, reference_graphs, key_prefix):
    ...
    metrics = {
        ...
        f"{key_prefix}/Graph/Edges_real": _safe_log_value(real_stats["edge_count"]),
        f"{key_prefix}/Graph/Edges_gen": _safe_log_value(gen_stats["edge_count"]),
        f"{key_prefix}/Graph/AvgDegree_real": _safe_log_value(real_stats["avg_degree"]),
        f"{key_prefix}/Graph/AvgDegree_gen": _safe_log_value(gen_stats["avg_degree"]),
    ...
    metrics[f"{key_prefix}/Graph/Edges_abs_gap"] = abs(
        metrics[f"{key_prefix}/Graph/Edges_gen"] - metrics[f"{key_prefix}/Graph/Edges_real"]
    )
    metrics[f"{key_prefix}/Graph/AvgDegree_abs_gap"] = abs(
        metrics[f"{key_prefix}/Graph/AvgDegree_gen"] - metrics[f"{key_prefix}/Graph/AvgDegree_real"]
    )
