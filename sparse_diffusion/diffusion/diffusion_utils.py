...
def sample_sparse_discrete_feature_noise_heterogeneous(limit_dist, node_mask, dataset_info, out_dims_E, device):
    ...
    # 仅当存在有效的 per-type 子类分布时才用「先类型再子类」；否则直接用全局 limit_dist.X，
    # 与评估用的 node_types 一致，避免 node_subtype_by_type 缺失时退化为类型内均匀导致 TV 很大
    use_two_step = (
        node_type_distribution
        and node_subtype_by_type
        and len(node_type_names) > 0
        and all(node_subtype_by_type.get(t) is not None for t in node_type_names)
    )
    if use_two_step:
    ...
    for b in range(bs):
        ...
        # 只考虑真实存在边的关系族对应的类型对（与训练、query 采样一致）
        type_pairs = set()
        for fam_name, endpoints in fam_endpoints.items():
            if edge_family_avg_edge_counts.get(fam_name, 0) <= 0:
                continue
...
def sample_sparse_discrete_feature_noise_heterogeneous_fixed_nodes(
    fixed_node_subtypes, batch, limit_dist, dataset_info, out_dims_E, device
):
    """固定节点、仅边从噪声采样。用于「给定 DBLP 节点、边从噪声初始化」的去噪边生成。
    fixed_node_subtypes: (N,) 全局子类别 ID
    batch: (N,) 节点所属图 ID
    """
    fixed_node_subtypes = fixed_node_subtypes.to(device).long()
    batch = batch.to(device)
    n_node = fixed_node_subtypes.shape[0]
    total_node_subtypes = int(limit_dist.X.shape[-1])
    node = F.one_hot(fixed_node_subtypes.clamp(0, total_node_subtypes - 1), num_classes=total_node_subtypes).float()
    node_t = fixed_node_subtypes

    bs = int(batch.max().item()) + 1
    n_nodes = torch.zeros(bs, dtype=torch.long, device=device)
    for b in range(bs):
        n_nodes[b] = (batch == b).sum()
    ptr = torch.zeros(bs + 1, dtype=torch.long, device=device)
    ptr[1:] = torch.cumsum(n_nodes, dim=0)

    y = torch.empty((bs, 0), dtype=torch.long, device=device)
    charge = node.new_zeros((*node.shape[:-1], 0))
    if limit_dist.charge.numel() > 0:
        charge_limit = limit_dist.charge[None, :].expand(n_node, -1).to(device)
        charge = charge_limit.multinomial(1)[:, 0]
    ...
    for b in range(bs):
        batch_mask = (batch == b)
        batch_node_t = node_t[batch_mask]
        batch_nodes_global = torch.where(batch_mask)[0]
        if batch_nodes_global.numel() == 0:
            continue
        graph_hash_base = int(batch_nodes_global.max().item()) + 1
        ...
        for src_type, dst_type in type_pairs:
            ...
            src_mask = (batch_node_t >= src_offset) & (batch_node_t < src_offset + src_size)
            dst_mask = (batch_node_t >= dst_offset) & (batch_node_t < dst_offset + dst_size)
            batch_src = batch_nodes_global[src_mask]
            batch_dst = batch_nodes_global[dst_mask]
            if len(batch_src) == 0 or len(batch_dst) == 0:
                continue
            num_src, num_dst = len(batch_src), len(batch_dst)
            if src_type == dst_type:
            ...
            if not fam_dist:
                ...
                total_edges_for_pair = min(
                    int(round(sum(edge_family_avg_edge_counts.get(fam_name, 0.0) for fam_name in fam_dist.keys()))),
                    num_possible
                )
                remaining_edges = total_edges_for_pair
                for fam_name, fam_prob in fam_dist.items():
