...
class DummyExtraFeatures:
    ...
    def __call__(self, noisy_data, sparse=True):
        ...
        if "X_t" in noisy_data:
            ...
            # 稀疏格式：不 densify（大图如 DBLP 11k 节点会 OOM），直接从 ptr/batch 推断形状创建空张量
            ptr = noisy_data.get("ptr")
            batch = noisy_data.get("batch")
            node_t = noisy_data.get("node_t")
            device = node_t.device if node_t is not None else torch.device("cpu")
            dtype = node_t.dtype if node_t is not None else torch.float32
            if ptr is not None and ptr.numel() > 1:
                n_node = int(ptr.diff().max().item())
                bs = int(ptr.shape[0] - 1)
            elif batch is not None and node_t is not None:
                bs = int(batch.max().item()) + 1
                if bs == 1:
                    n_node = int(node_t.shape[0])
                else:
                    n_node = int(batch.bincount().max().item())
            else:
                bs, n_node = 1, 0
            empty_x = torch.zeros(bs, n_node, 0, device=device, dtype=dtype)
            empty_e = torch.zeros(bs, n_node, n_node, 0, device=device, dtype=dtype)
            empty_y = torch.zeros(bs, 0, device=device, dtype=dtype)
            return utils.PlaceHolder(
                X=empty_x, E=empty_e, y=empty_y
            ), 0., 0.
...
class AdjacencyFeatures:
    ...
    def __call__(self, noisy_data):
        ...
        if self.heterogeneous:
            # 异质图：环/path 等关掉且不占维度，避免无效零特征
            kcyclesx = torch.zeros(bs, n, 0, device=device)
            kcyclesy_part = torch.zeros(bs, 0, device=device)
