...
def sampled_condensed_indices_uniformly(
    ...
    # torch.multinomial 最多支持 2^24 个类别，超过则走 randperm 通用路径
    _MULTINOMIAL_MAX_CATEGORIES = 2**24

    if (
        len(torch.unique(max_condensed_value)) == 1
        and len(torch.unique(num_edges_to_sample)) == 1
        and max_condensed_value[0].item() <= _MULTINOMIAL_MAX_CATEGORIES
    ):
...
def sample_query_edges(
    ...
    if batch_size == 1:
        ...
        # query batch should align with number of sampled edges, not number of nodes
        return edge_index, torch.zeros(edge_index.shape[1], dtype=torch.long, device=device)
    ...
    if len(torch.unique(num_nodes_per_graph)) == 1:
        ...
        # One graph id per sampled query edge (same as edge_batch from sampling).
